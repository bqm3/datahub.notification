...
{    
    public class S_DULIEU_FWDao : OracleBaseImpl<S_DULIEU_FW>
    {        
		protected override void SetInfoDerivedClass()
        {
            TableName = "S_DULIEU_FW";
            PackageName = "PK_S_DULIEU_FW";         
			ConnectionString = Config.CONFIGURATION_GLOBAL.Databases.ORACLE.SHARE_OWNER.ConnectionString;
        }
        #region User defined
        ...
        #region Base		
        public long? Insert(S_DULIEU_FW_Request request)
        {        
			var info = new S_DULIEU_FW
			{
				CODE = (request != null && !string.IsNullOrEmpty(request.CODE)) ? request.CODE : null,
                DATA = (request != null && !string.IsNullOrEmpty(request.DATA)) ? request.DATA : null,
		...
		public bool? Insert(List<S_DULIEU_FW_Request> request)
		{
			var list = new List<S_DULIEU_FW>();
			foreach (var info in request)
			{
				list.Add(new S_DULIEU_FW
				{
					CODE = (info != null && !string.IsNullOrEmpty(info.CODE)) ? info.CODE : null,
                    DATA = (info != null && !string.IsNullOrEmpty(info.DATA)) ? info.DATA : null,
		...
		public bool? UpdateList(List<S_DULIEU_FW> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpdateList(list,"ID", getPropertyNames);
			return result;
		}
		public bool? UpInsertList(List<S_DULIEU_FW> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpInsertList(list, "ID", getPropertyNames);
			return result;
		}
		public List<S_DULIEU_FW> GetList(Dictionary<string, object> dictParam)
        {
        ...
        public List<S_DULIEU_FW> GetListPaging(Dictionary<string, object> dictParam, DateTime? p_CDATE_START,
        ...
        {            
            ...
            List<S_DULIEU_FW> result = this.ExecuteQuery("SP_GET_LIST_PAGING", sqlParam, pageIndex, pageSize, ref totalRecord);
            return result;
        }
        ...
        public S_DULIEU_FW QuerySolr_GetInfo(string _ID, string _CODE)
        {
            ...
            string[] fieldSelect = { "ID","CODE","DATA","STATUS","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            int totalRecord = 0;
            List<S_DULIEU_FW> results = QuerySolrBase<S_DULIEU_FW>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"S_DULIEU_FW/", string.Empty, lstFilter, fieldSelect, lstOrder, 0, int.Parse(Config.SOLR_PAGE_SIZE), ref totalRecord);
            if (results != null && results.Count > 0)
                return results[0];
            return null;
        }

        public List<S_DULIEU_FW> QuerySolr_GetListPaging(string keyword,
			string p_CODE,
			string p_STATUS,
            ...
            int pageIndex, int pageSize, ref int totalRecord)
        {
            ...
            lstOrder.Add(new SortOrder("ID", Order.DESC));
            string[] fieldSelect = { "ID","CODE","DATA","STATUS","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            var results = QuerySolrBase<S_DULIEU_FW>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"S_DULIEU_FW/", keyword, lstFilter, fieldSelect, lstOrder, pageIndex - 1, pageSize, ref totalRecord);
            List<S_DULIEU_FW> listAddRange = new List<S_DULIEU_FW>();
            if (results != null)

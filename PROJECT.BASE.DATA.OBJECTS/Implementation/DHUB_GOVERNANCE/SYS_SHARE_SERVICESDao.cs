...
{    
    public class SYS_SHARE_SERVICESDao : OracleBaseImpl<SYS_SHARE_SERVICES>
    {        
		protected override void SetInfoDerivedClass()
        {
            TableName = "SYS_SHARE_SERVICES";
            PackageName = "PK_SYS_SHARE_SERVICES";         
        ...
        #region Base		
        public long? Insert(SYS_SHARE_SERVICES_Request request)
        {        
			var info = new SYS_SHARE_SERVICES
			{
		...
		public bool? Insert(List<SYS_SHARE_SERVICES_Request> request)
		{
			var list = new List<SYS_SHARE_SERVICES>();
			foreach (var info in request)
			{
				list.Add(new SYS_SHARE_SERVICES
				{
		...
		public bool? UpdateList(List<SYS_SHARE_SERVICES> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpdateList(list,"ID", getPropertyNames);
			return result;
		}
		public bool? UpInsertList(List<SYS_SHARE_SERVICES> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpInsertList(list, "ID", getPropertyNames);
			return result;
		}
		public List<SYS_SHARE_SERVICES> GetList(Dictionary<string, object> dictParam)
        {
        ...
        public List<SYS_SHARE_SERVICES> GetListPaging(Dictionary<string, object> dictParam, DateTime? p_CDATE_START,
        ...
        {            
            ...
            List<SYS_SHARE_SERVICES> result = this.ExecuteQuery("SP_GET_LIST_PAGING", sqlParam, pageIndex, pageSize, ref totalRecord);
            return result;
        }
        ...
        public SYS_SHARE_SERVICES QuerySolr_GetInfo(string _ID, string _CODE)
        {
            ...
            string[] fieldSelect = { "ID","CODE","NAME","PARENT_CODE","PARENT_NAME","DESCRIPTION","IS_ACTIVE","STATUS","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            int totalRecord = 0;
            List<SYS_SHARE_SERVICES> results = QuerySolrBase<SYS_SHARE_SERVICES>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"SYS_SHARE_SERVICES/", string.Empty, lstFilter, fieldSelect, lstOrder, 0, int.Parse(Config.SOLR_PAGE_SIZE), ref totalRecord);
            if (results != null && results.Count > 0)
                return results[0];
            return null;
        }

        public List<SYS_SHARE_SERVICES> QuerySolr_GetListPaging(string keyword,
			string p_CODE,
            ...
            int pageIndex, int pageSize, ref int totalRecord)
        {
            ...
            lstOrder.Add(new SortOrder("ID", Order.DESC));
            string[] fieldSelect = { "ID","CODE","NAME","PARENT_CODE","PARENT_NAME","DESCRIPTION","IS_ACTIVE","STATUS","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            var results = QuerySolrBase<SYS_SHARE_SERVICES>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"SYS_SHARE_SERVICES/", keyword, lstFilter, fieldSelect, lstOrder, pageIndex - 1, pageSize, ref totalRecord);
            List<SYS_SHARE_SERVICES> listAddRange = new List<SYS_SHARE_SERVICES>();
            if (results != null)

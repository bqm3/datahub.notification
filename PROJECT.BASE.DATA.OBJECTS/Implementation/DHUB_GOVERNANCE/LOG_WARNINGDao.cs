...
{    
    public class LOG_WARNINGDao : OracleBaseImpl<LOG_WARNING>
    {        
		protected override void SetInfoDerivedClass()
        {
            TableName = "LOG_WARNING";
            PackageName = "PK_LOG_WARNING";         
        ...
        #region Base		
        public long? Insert(LOG_WARNING_Request request)
        {        
			var info = new LOG_WARNING
			{
		...
		public bool? Insert(List<LOG_WARNING_Request> request)
		{
			var list = new List<LOG_WARNING>();
			foreach (var info in request)
			{
				list.Add(new LOG_WARNING
				{
		...
		public bool? UpdateList(List<LOG_WARNING> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpdateList(list,"ID", getPropertyNames);
			return result;
		}
		public bool? UpInsertList(List<LOG_WARNING> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpInsertList(list, "ID", getPropertyNames);
			return result;
		}
		public List<LOG_WARNING> GetList(Dictionary<string, object> dictParam)
        {
        ...
        public List<LOG_WARNING> GetListPaging(Dictionary<string, object> dictParam, DateTime? p_CDATE_START,
        ...
        {            
            ...
            List<LOG_WARNING> result = this.ExecuteQuery("SP_GET_LIST_PAGING", sqlParam, pageIndex, pageSize, ref totalRecord);
            return result;
        }
        ...
        public LOG_WARNING QuerySolr_GetInfo(string _ID, string _CODE)
        {
            ...
            string[] fieldSelect = { "ID","CODE","LOG_TYPE_CODE","LOG_ACTION_ID","MESSAGE_LOG","MESSAGE_DETAIL","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            int totalRecord = 0;
            List<LOG_WARNING> results = QuerySolrBase<LOG_WARNING>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"LOG_WARNING/", string.Empty, lstFilter, fieldSelect, lstOrder, 0, int.Parse(Config.SOLR_PAGE_SIZE), ref totalRecord);
            if (results != null && results.Count > 0)
                return results[0];
            return null;
        }

        public List<LOG_WARNING> QuerySolr_GetListPaging(string keyword,
			string p_CODE,
			string p_LOG_TYPE_CODE,			string p_LOG_ACTION_ID,			string p_MESSAGE_LOG,			string p_MESSAGE_DETAIL,
            ...
            int pageIndex, int pageSize, ref int totalRecord)
        {
            ...
            lstOrder.Add(new SortOrder("ID", Order.DESC));
            string[] fieldSelect = { "ID","CODE","LOG_TYPE_CODE","LOG_ACTION_ID","MESSAGE_LOG","MESSAGE_DETAIL","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            var results = QuerySolrBase<LOG_WARNING>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"LOG_WARNING/", keyword, lstFilter, fieldSelect, lstOrder, pageIndex - 1, pageSize, ref totalRecord);
            List<LOG_WARNING> listAddRange = new List<LOG_WARNING>();
            if (results != null)

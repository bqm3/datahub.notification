...
{    
    public class SYS_DST_TYPEDao : OracleBaseImpl<SYS_DST_TYPE>
    {        
		protected override void SetInfoDerivedClass()
        {
            TableName = "SYS_DST_TYPE";
            PackageName = "PK_SYS_DST_TYPE";         
        ...
        #region Base		
        public long? Insert(SYS_DST_TYPE_Request request)
        {        
			var info = new SYS_DST_TYPE
			{
		...
		public bool? Insert(List<SYS_DST_TYPE_Request> request)
		{
			var list = new List<SYS_DST_TYPE>();
			foreach (var info in request)
			{
				list.Add(new SYS_DST_TYPE
				{
		...
		public bool? UpdateList(List<SYS_DST_TYPE> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpdateList(list,"ID", getPropertyNames);
			return result;
		}
		public bool? UpInsertList(List<SYS_DST_TYPE> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpInsertList(list, "ID", getPropertyNames);
			return result;
		}
		public List<SYS_DST_TYPE> GetList(Dictionary<string, object> dictParam)
        {
        ...
        public List<SYS_DST_TYPE> GetListPaging(Dictionary<string, object> dictParam, DateTime? p_CDATE_START,
        ...
        {            
            ...
            List<SYS_DST_TYPE> result = this.ExecuteQuery("SP_GET_LIST_PAGING", sqlParam, pageIndex, pageSize, ref totalRecord);
            return result;
        }
        ...
        public SYS_DST_TYPE QuerySolr_GetInfo(string _ID, string _CODE)
        {
            ...
            int totalRecord = 0;
            List<SYS_DST_TYPE> results = QuerySolrBase<SYS_DST_TYPE>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"SYS_DST_TYPE/", string.Empty, lstFilter, fieldSelect, lstOrder, 0, int.Parse(Config.SOLR_PAGE_SIZE), ref totalRecord);
            if (results != null && results.Count > 0)
                return results[0];
            return null;
        }

        public List<SYS_DST_TYPE> QuerySolr_GetListPaging(string keyword,
			string p_CODE,
            ...
            int pageIndex, int pageSize, ref int totalRecord)
        {
            ...
            lstOrder.Add(new SortOrder("ID", Order.DESC));
            string[] fieldSelect = { "ID","CODE","NAME","NOTE","IS_ACTIVE","STATUS","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            var results = QuerySolrBase<SYS_DST_TYPE>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"SYS_DST_TYPE/", keyword, lstFilter, fieldSelect, lstOrder, pageIndex - 1, pageSize, ref totalRecord);
            List<SYS_DST_TYPE> listAddRange = new List<SYS_DST_TYPE>();
            if (results != null)

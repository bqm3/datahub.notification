...
{    
    public class SYS_CONFIG_METHODDao : OracleBaseImpl<SYS_CONFIG_METHOD>
    {        
		protected override void SetInfoDerivedClass()
        {
            TableName = "SYS_CONFIG_METHOD";
            PackageName = "PK_SYS_CONFIG_METHOD";         
        ...
        #region Base		
        public long? Insert(SYS_CONFIG_METHOD_Request request)
        {        
			var info = new SYS_CONFIG_METHOD
			{
		...
		public bool? Insert(List<SYS_CONFIG_METHOD_Request> request)
		{
			var list = new List<SYS_CONFIG_METHOD>();
			foreach (var info in request)
			{
				list.Add(new SYS_CONFIG_METHOD
				{
		...
		public bool? UpdateList(List<SYS_CONFIG_METHOD> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpdateList(list,"ID", getPropertyNames);
			return result;
		}
		public bool? UpInsertList(List<SYS_CONFIG_METHOD> list)
		{
			var getPropertyNames = this.GetPropertyNames();
			var result = this.UpInsertList(list, "ID", getPropertyNames);
			return result;
		}
		public List<SYS_CONFIG_METHOD> GetList(Dictionary<string, object> dictParam)
        {
        ...
        public List<SYS_CONFIG_METHOD> GetListPaging(Dictionary<string, object> dictParam, DateTime? p_CDATE_START,
        ...
        {            
            ...
            List<SYS_CONFIG_METHOD> result = this.ExecuteQuery("SP_GET_LIST_PAGING", sqlParam, pageIndex, pageSize, ref totalRecord);
            return result;
        }
        ...
        public SYS_CONFIG_METHOD QuerySolr_GetInfo(string _ID, string _CODE)
        {
            ...
            int totalRecord = 0;
            List<SYS_CONFIG_METHOD> results = QuerySolrBase<SYS_CONFIG_METHOD>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"SYS_CONFIG_METHOD/", string.Empty, lstFilter, fieldSelect, lstOrder, 0, int.Parse(Config.SOLR_PAGE_SIZE), ref totalRecord);
            if (results != null && results.Count > 0)
                return results[0];
            return null;
        }

        public List<SYS_CONFIG_METHOD> QuerySolr_GetListPaging(string keyword,
			string p_CODE,
            ...
            int pageIndex, int pageSize, ref int totalRecord)
        {
            ...
            lstOrder.Add(new SortOrder("ID", Order.DESC));
            string[] fieldSelect = { "ID","CODE","NAME","NOTE","IS_ACTIVE","STATUS","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            var results = QuerySolrBase<SYS_CONFIG_METHOD>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"SYS_CONFIG_METHOD/", keyword, lstFilter, fieldSelect, lstOrder, pageIndex - 1, pageSize, ref totalRecord);
            List<SYS_CONFIG_METHOD> listAddRange = new List<SYS_CONFIG_METHOD>();
            if (results != null)

...
{    
    public class ITG_CATEGORYDao : OracleBaseImpl<ITG_CATEGORY>
    {        
		protected override void SetInfoDerivedClass()
        {
            TableName = "ITG_CATEGORY";
            PackageName = "PK_ITG_CATEGORY";
        ...
        #region Base		
        public long? Insert(ITG_CATEGORY_Information info)
        {        
			var paramValue = new ITG_CATEGORY
			{
		...
		public List<ITG_CATEGORY> GetList(Dictionary<string, object> dictParam)
        {
        ...
        public List<ITG_CATEGORY> GetListPaging(Dictionary<string, object> dictParam, DateTime? p_CDATE_START,
        ...
        {            
            ...
            List<ITG_CATEGORY> result = this.ExecuteQuery("SP_GET_LIST_PAGING", sqlParam, pageIndex, pageSize, ref totalRecord);
            return result;
        }
        ...
        public ITG_CATEGORY QuerySolr_GetInfo(string _ID, string _CODE)
        {
            ...
            string[] fieldSelect = { "ID","CODE","NAME","PARENT_CODE","PARENT_NAME","DESCRIPTION","STATUS","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            int totalRecord = 0;
            List<ITG_CATEGORY> results = QuerySolrBase<ITG_CATEGORY>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"ITG_CATEGORY/", string.Empty, lstFilter, fieldSelect, lstOrder, 0, int.Parse(Config.SOLR_PAGE_SIZE), ref totalRecord);
            if (results != null && results.Count > 0)
                return results[0];
            return null;
        }

        public List<ITG_CATEGORY> QuerySolr_GetListPaging(string keyword,
			string p_CODE,
			string p_NAME,			string p_PARENT_CODE,			string p_PARENT_NAME,			string p_DESCRIPTION,			string p_STATUS,
            ...
            int pageIndex, int pageSize, ref int totalRecord)
        {
            ...
            lstOrder.Add(new SortOrder("ID", Order.DESC));
            string[] fieldSelect = { "ID","CODE","NAME","PARENT_CODE","PARENT_NAME","DESCRIPTION","STATUS","IS_DELETE","CDATE","LDATE","CUSER","LUSER" };
            var results = QuerySolrBase<ITG_CATEGORY>.QuerySolr_GetList(Config.SOLR_URL_CORE_BASE + $"ITG_CATEGORY/", keyword, lstFilter, fieldSelect, lstOrder, pageIndex - 1, pageSize, ref totalRecord);
            List<ITG_CATEGORY> listAddRange = new List<ITG_CATEGORY>();
            if (results != null)

...
{  
    public class SEND_SMSDao : MongoBaseImpl<SEND_SMSInfo>
    {
        protected static int FindMaxTime = 0;
        protected override void SetInfoDerivedClass()
        {
            CollectionName = "SEND_SMS";
        ...
        #endregion
        public bool SEND_SMS_Update(SEND_SMSInfo info)
        {
            ...
            var result = this.UpdateOne(filter,
                Builders<BsonDocument>.Update
                      .Set("TO_PHONE", info.TO_PHONE)
                      .Set("SMS_CONTENT_TYPE", info.SMS_CONTENT_TYPE)
                      .Set("VERIFY_CODE", info.VERIFY_CODE)
                      .Set("TIMESTAMP", info.TIMESTAMP)
                      .Set("TYPE_SMS", info.TYPE_SMS)
        ...
        public bool SEND_SMS_Delete(string _CODE)
        {
        ...
        #region Get List               
        public SEND_SMSInfo SEND_SMS_GetInfo(string _CODE)
        {
            BsonDocument filter = new BsonDocument();
            filter.Add("CODE", _CODE);
            var result = this.Select<SEND_SMSInfo>(filter);
            if (result != null && result.Count > 0)
                return result[0];
            return null;

        }
        public List<SEND_SMSInfo> SEND_SMS_GetList(int _STATUS)
        {
            try
            {
                BsonDocument filter = new BsonDocument();
        ...
        public List<SEND_SMSInfo> SEND_SMS_GetList()
        {
            try
            {
                BsonDocument filter = new BsonDocument();
        ...
        public async Task<DYNAMICInfo<Dictionary<string, object>>> SEND_SMS_GetAllWithPadding(List<DATA_FIELDInfo> listFilter, int pageIndex, int pageSize)
        {
            BsonDocument filter = new BsonDocument();

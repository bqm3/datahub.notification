...
{
    public class LINE_CHARTDao : MongoBaseImpl<LINE_CHART>
    {
        ...
        #endregion           
        public bool Update(LINE_CHART info)
        {
            ...
            var result = this.UpdateOne(filter,
                Builders<BsonDocument>.Update
                      ...
                      .Set("COLOR_CODE", info.COLOR_CODE)
        ...
        #region Get List              
        public LINE_CHART GetInfo(string CODE)
        {
            BsonDocument filter = new BsonDocument();
            filter.Add("CODE", CODE);
            var result = this.Select<LINE_CHART>(filter);
            if (result != null && result.Count > 0)
                return result[0];
            return null;

        }
        public List<LINE_CHART> GetList()
        {
            BsonDocument filter = new BsonDocument();
            var result = this.Select<LINE_CHART>(filter);
            return result;
        ...
        public List<LINE_CHART> GetList(string _SYSTEM_CODE, string _SERVICE_CODE, string _SERVICE_TYPE, string _REPORT_TYPE, string _DATE_START, string _DATE_END)
        {
            ...
            if (!string.IsNullOrEmpty(_SYSTEM_CODE))
            {
                string[] array = Regex.Split(_SYSTEM_CODE, ",");
                if (array.Length == 1)
                    listFilter.Add(new DATA_FIELD { FIELD_NAME = "SYSTEM_CODE", FIELD_VALUE = _SYSTEM_CODE, DATA_TYPE = "$string" });
                else
                    listFilter.Add(new DATA_FIELD { FIELD_NAME = "SYSTEM_CODE", FIELD_VALUE = array, DATA_TYPE = "$in-string" });
            }
            if (!string.IsNullOrEmpty(_SERVICE_CODE))
            {
                string[] array = Regex.Split(_SERVICE_CODE, ",");
                if (array.Length == 1)
                    listFilter.Add(new DATA_FIELD { FIELD_NAME = "SERVICE_CODE", FIELD_VALUE = _SERVICE_CODE, DATA_TYPE = "$string" });
                else
                    listFilter.Add(new DATA_FIELD { FIELD_NAME = "SERVICE_CODE", FIELD_VALUE = array, DATA_TYPE = "$in-string" });
            }
            if (!string.IsNullOrEmpty(_SERVICE_TYPE))
            {                
                string[] array = Regex.Split(_SERVICE_TYPE, ",");
                if (array.Length == 1)
                    listFilter.Add(new DATA_FIELD { FIELD_NAME = "SERVICE_TYPE", FIELD_VALUE = _SERVICE_TYPE, DATA_TYPE = "$string" });
                else
                    listFilter.Add(new DATA_FIELD { FIELD_NAME = "SERVICE_TYPE", FIELD_VALUE = array, DATA_TYPE = "$in-string" });
            }
            if (!string.IsNullOrEmpty(_REPORT_TYPE))
            {
                string[] array = Regex.Split(_REPORT_TYPE, ",");
                if (array.Length == 0)
                    listFilter.Add(new DATA_FIELD { FIELD_NAME = "REPORT_TYPE", FIELD_VALUE = _REPORT_TYPE, DATA_TYPE = "$string" });
                else
                    listFilter.Add(new DATA_FIELD { FIELD_NAME = "REPORT_TYPE", FIELD_VALUE = array, DATA_TYPE = "$in-string" });
            }

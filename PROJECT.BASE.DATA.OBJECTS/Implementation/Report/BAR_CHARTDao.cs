...
{
    public class BAR_CHARTDao : MongoBaseImpl<BAR_CHART>
    {
        ...
        #endregion           
        public bool Update(BAR_CHART info)
        {
        ...
        #region Get List              
        public BAR_CHART GetInfo(string CODE)
        {
            BsonDocument filter = new BsonDocument();
            filter.Add("CODE", CODE);            
            var result = this.Select<BAR_CHART>(filter);
            if (result != null && result.Count > 0)
                return result[0];
            return null;

        }
        public List<BAR_CHART> GetList()
        {
            BsonDocument filter = new BsonDocument();
            var result = this.Select<BAR_CHART>(filter);
            return result;
        ...
        public List<BAR_CHART> GetList(string _SYSTEM_CODE, string _SERVICE_CODE, string _SERVICE_TYPE, string _REPORT_TYPE, string _DATE_START, string _DATE_END)
        {
